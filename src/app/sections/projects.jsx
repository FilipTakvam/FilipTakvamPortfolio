...
const Projects = ({ navbarHeight }) => {
  const projects = [
    ...
    { name: 'Willys self checkout', link: '/' },
  ...
  return (
    <section className={`${styles.projectsSection} acceleratedRendering`} ref={container} id="projects">
            ...
            <motion.ul variants={itemsContainer} initial="hidden" animate={animationControls}>
              {projects.map((project, index) => (
                <motion.li key={index} variants={items}>
                  <Link
                    href={project.link}
                    className={index === activeProject ? `${styles.activeLink} ${styles.link} ` : styles.link}
                  >
                    {project.name}
                  </Link>
                </motion.li>
      ...
      <div ref={inViewRef2} className={`${styles.projectsContainer} wrapper ${styles.mobile}`} style={{ top: `${0}px` }}>
        <div className={`${styles.projectsListContainer}`} style={{ height: `calc(100svh - ${navbarHeight}px)` }}>
          <div className={styles.projectsList}>
            <h2>Projects</h2>
            <motion.ul variants={itemsContainer} initial="hidden" animate={animationControls}>
